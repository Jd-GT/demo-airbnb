...
class TenantBootstrapTests(APITestCase):
    def test_creating_tenant_seeds_roles_and_primary_owner(self):
        ...
        self.assertSetEqual(set(roles.values_list("name", flat=True)), {"Admin Total", "Solo Lectura"})
...
class OwnerProtectionTests(APITestCase):
    def test_primary_owner_cannot_be_deactivated_or_downgraded(self):
        ...
        deactivate_response = self.client.patch(url, {"is_active": False}, format="json")
...
class TenantIsolationAndRBAC(APITestCase):
    ...
    def test_readonly_role_cannot_write_inventory(self):
        readonly_role = TenantRole.all_objects.get(tenant=self.tenant_a, name="Solo Lectura")
        ...
        list_response = self.client.get(f"/api/tenants/{self.tenant_a.id}/inventory/properties/")
